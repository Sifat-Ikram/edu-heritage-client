...
export default function ProgramCard(program: UndergraduateProgram) {
  ...
  return (
    <motion.div
      ...
      className="overflow-hidden rounded-2xl bg-white dark:bg-[#161929] dark:border-[2px] border-solid border-white shadow-xl hover:shadow-2xl transition-shadow"
    >
      ...
      <div className="p-4">
        <h3 className="text-lg font-semibold text-teal-800 dark:text-white">{program.title}</h3>
        <p className="text-sm text-gray-600 dark:text-white mt-1">{program.description}</p>
      </div>
    </motion.div>
  );
}
